                        ...
                        >
                            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
                                ...
                                <div className="md:col-span-3 flex items-center gap-4">
                                    <Text variant="mono" size="xs" className="text-foreground/20">/{article.id}</Text>
                                    <Text size="xs" className={cn("font-medium uppercase tracking-wider", article.accent)}>
                                        {article.category}
                                    </Text>
                                </div>
                                ...
                                <div className="md:col-span-6">
                                    <Link href={`/blog/${article.slug}`}>
                                        <Heading level={3} className="text-2xl md:text-4xl group-hover:text-foreground/70 transition-colors duration-300">
                                            {article.title}
                                        </Heading>
                                    </Link>
                                </div>
                                ...
                                <div className="md:col-span-3 flex items-center justify-between md:justify-end gap-8">
                                    <div className="flex flex-col items-end gap-1">
                                        <Text size="sm" variant="muted">{article.date}</Text>
                                        <div className="flex items-center gap-2">
                                            <Clock className="w-3 h-3" />
                                            <Text size="xs" variant="muted">{article.readTime} read</Text>
                                        </div>
                                    </div>

                                    <Link href={`/blog/${article.slug}`} className="w-12 h-12 rounded-full border border-[var(--glass-border)] flex items-center justify-center group-hover:bg-accent group-hover:text-accent-foreground transition-all duration-300 transform group-hover:rotate-45" aria-label={`Read article: ${article.title}`}>
